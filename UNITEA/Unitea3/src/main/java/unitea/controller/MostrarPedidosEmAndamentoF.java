package unitea.controller;

import jakarta.servlet.ServletException;
import jakarta.servlet.annotation.WebServlet;
import jakarta.servlet.http.HttpServlet;
import jakarta.servlet.http.HttpServletRequest;
import jakarta.servlet.http.HttpServletResponse;
import jakarta.servlet.http.HttpSession;
import unitea.dao.MostrarPedidosEmAndamentoFDao;
import unitea.model.PedidoMonitoriaModel;

import java.io.IOException;
import java.util.List;

/**
 * Servlet implementation class MostrarPedidosEmAndamentoF
 */
//@WebServlet("/MostrarPedidosEmAndamentoF")
public class MostrarPedidosEmAndamentoF extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public MostrarPedidosEmAndamentoF() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		response.getWriter().append("Served at: ").append(request.getContextPath());
		
		request.setCharacterEncoding("UTF-8");
		response.setContentType("text/html; charset=UTF-8");
		
		HttpSession session = request.getSession();
		int idFamiliar= (Integer) session.getAttribute("id_familiar");
		
		MostrarPedidosEmAndamentoFDao tela= new MostrarPedidosEmAndamentoFDao();
		
		List<PedidoMonitoriaModel> listaDePedidos =tela.listarPedidosTelaInicial(idFamiliar); 
		session.setAttribute("pedidos2", listaDePedidos);
		
		
		/*request.setAttribute("pedidos", listaDePedidos);*/
	
		response.sendRedirect("telaFamiliar.jsp");
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		doGet(request, response);
	}

}
